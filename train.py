...
try:  # Mixed precision training https://github.com/NVIDIA/apex
    from apex import amp  ## 基于PyTorch的混合精度训练神器（双倍训练速度）
except:
...
def train(hyp):
    ...
    accumulate = max(round(64 / batch_size), 1)  # accumulate n times before optimizer update (bs 64) 过accumulate个batch才更新参数
    weights = opt.weights  # initial training weights
    imgsz_min, imgsz_max, imgsz_test = opt.img_size  # img sizes (min, max, test)

    # Image Sizes
    gs = 32  # (pixels) grid size
    assert math.fmod(imgsz_min, gs) == 0, '--img-size %g must be a %g-multiple' % (imgsz_min, gs) # 32的倍数
    opt.multi_scale |= imgsz_min != imgsz_max  # multi if different (min, max)  多尺度训练
    if opt.multi_scale:
    ...
    # Initialize model
    model = Darknet(cfg).to(device)  ## 注意顺序，先把model转到GPU，才能把参数传给optim
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        try:
            ...
            ## strict=False用法：当model中有自己添加的层，而预训练模型没有对应的值，就用strict=False 表示只加载对应上的值。
    ...
    # 固定除 YOLOLayer 和 YOLOLayer 下一层的所有参数
    ...
    # 补充：DistributedDataParallel 简称DDP，DataParallel简称DP
    #      DP适用于单机多卡，DDP适用于单机多卡和多机多卡，速度比DP快
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')  ## 访问google云存储的
    ...
    if not opt.evolve:  # Train normally
        ...
        for _ in range(1):  # generations to evolve  遗传算法选择hyp参数
